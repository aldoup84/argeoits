...
public class RayCastCF : MonoBehaviour {
	string target;
	int encontradas, consecutivas, puntos, errores, final, idSesion;
    ...
    void Start () {				// Use this for initialization
		...
		idSesion = PlayerPrefs.GetInt("idSesion");
				...
				//	Debug.Log ("Detectado: " + hitInfo.collider.gameObject.name + ", Objetivo: "+ target);
					...
					//	Debug.Log ("Encontrada la parte " + consecutivas + " de 2 " + nom + ". Final " + final);
						...
						if (consecutivas == 2) {	
							...
							//Debug.Log(Math.Abs(ant - level));
							//Debug.Log("Anterior: " + ant + "	Nuevo  " + level);
							if (ant != level) {
								Save();
							}
					...
					if (final >= 10) {
                      ...
                      // Debug.Log("Errores " + err);                        
                        GuardarDatos g = new GuardarDatos("3", idSesion, aciertos, err, puntos, tiempo, GetNivel(), GetDificultad(level));
                        g.Insert();
	...
	public string GetDificultad(int level) {
		string res = "Facil";
		switch (level) {
			case 1: res = "Facil"; break;
			case 2: res = "Regular"; break;
			case 3: res = "Avanzado"; break;
		}
		return res;
	}
  ...
  //      Debug.Log(target);
	...
	public void Save() {
		int err = Convert.ToInt32(GameObject.Find("txtErrores").GetComponent<Text>().text);
		int aciertos = errores = Convert.ToInt32(GameObject.Find("txtAciertos").GetComponent<Text>().text);
		int niv = GetNivel();
		GuardarDatos g = new GuardarDatos("3", idSesion, aciertos, err, puntos, tiempo, niv, GetDificultad(niv));
		g.Insert();
		Debug.Log("Se han guardado los datos");
	}
	public int GetNivel() {
		return Convert.ToInt32(GameObject.Find("txtNivel").GetComponent<Text>().text);
	}
}