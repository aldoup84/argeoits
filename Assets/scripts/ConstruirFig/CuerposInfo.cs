...
*/
    ...
    public DatosFigura GetNextItem(int Nivel){
        ...
        if (terminados.Count > 0) { 
            ...
            Debug.Log("Elementos restantes: " + terminados.Count + "Nivel : " + Nivel  + "  " + temp.Count());
            if (temp.Count() > 0){
				int r = new System.Random().Next(0, temp.Count()-1);
				df = temp.ElementAt(r);
                Debug.Log("Proximo Blanco:  " + r + ",  " + df.Descripcion);
            }
            else
            {
